...
const SESSION_ID_KEY = 'legal-cdp-session-id';
const SESSION_EVENTS_KEY = 'legal-cdp-session-events';
const VISIT_COUNT_KEY = 'legal-cdp-visit-count';
const VISIT_FLAG_KEY = 'legal-cdp-visit-recorded';
...
function derivePageContext(path: string): Record<string, string> {
  ...
  if (normalized.includes('/people')) context.category = 'People';
  if (normalized.includes('capital-markets') || normalized.includes('/industries/'))
    context.category = 'Capital Markets';
  if (normalized.includes('perspectives')) context.category = 'Perspectives';
  if (normalized.includes('expertise') || normalized.includes('/industries'))
    context.category = 'Expertise';
  if (normalized.includes('announcement')) context.category = 'Announcements';
  if (normalized.includes('elisabeth-plakinger')) context.collection = 'Elisabeth Plakinger';
  if (normalized.includes('about')) context.intent = 'Brand';
  if (normalized.includes('careers')) context.intent = 'Careers';
...
export function deriveAffinityFromEvents(
  ...
  for (const [key, val] of Object.entries(scores)) {
    ...
    if (['People', 'Capital Markets', 'Perspectives', 'Expertise', 'Announcements'].includes(key)) {
      categories[key] = normalized;
    } else if (key === 'Elisabeth Plakinger') {
