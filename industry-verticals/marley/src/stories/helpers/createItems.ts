...
export const createMockArticles = (count: number): Article[] =>
  Array.from({ length: count }).map((_, i) => ({
    ...
    fields: {
      ...
      Category: {
        id: `category-${i % 2}`,
        displayName: i % 2 === 0 ? 'Roofing' : 'Solar PV',
        name: i % 2 === 0 ? 'Roofing' : 'Solar PV',
        url: `/categories/${i % 2 === 0 ? 'roofing' : 'solar-pv'}`,
        fields: { Category: createTextField(i % 2 === 0 ? 'Roofing' : 'Solar PV') },
      } as Category,
    },
  }));
