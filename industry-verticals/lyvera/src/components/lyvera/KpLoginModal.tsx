...
import { KP_DEMO_EMAIL, useKpAuth } from '@/lib/kp-auth';
import { CdpSubscribeButton } from '@/components/cdp-profile-panel/CdpSubscribeButton';

export function KpLoginModal(): JSX.Element | null {
  const {
    loginOpen,
    loginStep,
    closeLogin,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="kp-login-title"
    >
      <div className="kp-login-modal__card w-full max-w-md rounded-sm bg-white p-8 shadow-xl md:p-10">
        <p className="mb-8 text-2xl font-bold tracking-tight text-[#1a3d2e]">Keith Prowse</p>

        {loginStep === 'account' ? (
          <>
            <h2 id="kp-login-title" className="m-0 text-xl font-bold text-[#1a1a1a]">
              Choose an account
            </h2>
            <p className="mt-2 mb-6 text-sm text-[#6b6b6b]">
              Sign in to access your Keith Prowse account
            </p>
          ...
          <>
            ...
            <p className="mt-2 mb-6 text-sm text-[#6b6b6b]">
              We just sent a code to {KP_DEMO_EMAIL}
            </p>
            <form onSubmit={onCodeSubmit}>
              <label htmlFor="kp-login-code" className="sr-only">
                Enter code
              </label>
              <input
                id="kp-login-code"
