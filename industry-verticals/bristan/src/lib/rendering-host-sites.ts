...
/** Sites served by the bristan rendering host (see docs/BRISTAN.md). */
export const BRISTAN_RENDERING_HOST_SITES = ['bristan', 'heritage'] as const;

/**
 * Site names to include in SSG path discovery for this rendering host.
 * sites.json from XM Cloud lists every tenant site; this host only builds bristan + heritage.
 */
export function getStaticBuildSiteNames(allSites: SiteInfo[]): string[] {
  ...
  const siteNames = configured?.length ? configured : [...BRISTAN_RENDERING_HOST_SITES];
...
export function getDefaultStaticBuildSite(): string {
  return scConfig.defaultSite || BRISTAN_RENDERING_HOST_SITES[0];
}
