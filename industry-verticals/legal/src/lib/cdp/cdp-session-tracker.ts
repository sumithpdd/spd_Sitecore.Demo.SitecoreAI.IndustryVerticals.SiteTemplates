...
function derivePageContext(path: string): Record<string, string> {
  ...
  if (normalized.includes('restructuring')) context.category = 'Restructuring';
  if (normalized.includes('out-law')) context.category = 'Out-Law';
  if (normalized.includes('expertise')) context.category = 'Expertise';
  if (normalized.includes('announcement')) context.category = 'Announcements';
  if (normalized.includes('dawn-allen')) context.collection = 'Dawn Allen';
...
export function deriveAffinityFromEvents(
  ...
  for (const [key, val] of Object.entries(scores)) {
    ...
    if (['People', 'Restructuring', 'Out-Law', 'Expertise', 'Announcements'].includes(key)) {
      categories[key] = normalized;
    } else if (key === 'Dawn Allen') {
