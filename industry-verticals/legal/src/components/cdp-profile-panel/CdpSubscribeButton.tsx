...
export function CdpSubscribeButton({
  ...
  const inputClass =
    variant === 'forma'
      ? 'mb-2 w-full rounded-lg border border-[#e8e8e8] bg-white px-4 py-2.5 text-sm text-[#1e1e1e] placeholder:text-[#9f9f9f] focus:border-[#7c0a2e] focus:ring-2 focus:ring-[#7c0a2e]/25 focus:outline-none'
  ...
  const successClass =
    variant === 'forma'
      ? 'w-full rounded-lg bg-[#f6f6f6] px-4 py-2.5 text-sm font-semibold text-[#7c0a2e]'
    ...
    <div className={className}>
      ...
      {error ? (
        <p
          className={cn(
            'mb-2 text-[12px]',
            variant === 'forma' ? 'text-danger' : 'text-yellow-200'
          )}
        >
          {error}
        </p>
