...
const AUTH_KEY = 'astonmartin-demo-auth';

/** Demo Owners Club persona (James — existing owner story). */
export const DEMO_EMAIL = 'james.owner@sitecore.net';
export const DEMO_DISPLAY_NAME = 'James';
...
export function DemoAuthProvider({ children }: { children: ReactNode }) {
  ...
  const [siteLabel, setSiteLabel] = useState('Aston Martin');
  ...
  const completeLogin = useCallback(() => {
    void (async () => {
      try {
        await identifyVisitorByEmail(DEMO_EMAIL, { asOwner: true });
      } catch (e) {
  ...
  const loginWithEmail = useCallback(
    async (email: string) => {
      await identifyVisitorByEmail(email, { asOwner: true });
