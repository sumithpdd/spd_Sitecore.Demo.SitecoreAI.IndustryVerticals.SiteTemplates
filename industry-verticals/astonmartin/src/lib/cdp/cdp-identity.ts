...
export function displayNameFromEmail(email: string): string {
  ...
  const first = capitalize(firstPart) || 'Owner';
...
export async function identifyVisitorByEmail(
  ...
  const [localPart] = value.split('@');
  const [firstPart, ...rest] = localPart.split(/[._-]+/).filter(Boolean);
  const firstName = capitalize(firstPart) || 'Owner';
  const lastName = rest.map(capitalize).join(' ');
  const asOwner = options?.asOwner ?? false;
  ...
  await identity({
    ...
    extensionData: {
      brand: 'Aston Martin',
      industry: 'Automobile',
      ...(asOwner ? { persona: 'Owner', category: 'Owners', intent: 'Ownership' } : {}),
    },
  });

  persistIdentifiedUser({ email: value, firstName, lastName });
  recordIdentityEvent(value, asOwner);
}
