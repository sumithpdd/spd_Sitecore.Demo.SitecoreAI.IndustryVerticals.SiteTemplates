...
/** Sites served by the university rendering host. */
export const UNIVERSITY_RENDERING_HOST_SITES = ['university'] as const;

/**
 * Site names to include in SSG path discovery for this rendering host.
 * sites.json from XM Cloud lists every tenant site; this host only builds university.
 */
export function getStaticBuildSiteNames(allSites: SiteInfo[]): string[] {
  ...
  const siteNames = configured?.length ? configured : [...UNIVERSITY_RENDERING_HOST_SITES];
...
export function getDefaultStaticBuildSite(): string {
  return scConfig.defaultSite || UNIVERSITY_RENDERING_HOST_SITES[0];
}
