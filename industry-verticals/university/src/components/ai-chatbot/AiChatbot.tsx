...
function isHomePath(asPath: string): boolean {
  const path = asPath.split('?')[0];
  return path === '/' || path === '';
}

/**
 * ChatGPT-style search assistant on the homepage. Answers from predefined Q&A plus the dummy Sitecore Search index.
 */
export default function AiChatbot(): JSX.Element | null {
  ...
  const onHome = router.isReady && isHomePath(router.asPath);

  useEffect(() => {
    if (!router.isReady || !onHome) return;
    const params = new URLSearchParams(router.asPath.split('?')[1] || '');
    if (params.get('utm_source')?.toLowerCase() === 'chatgpt') {
      setOpen(true);
    }
  }, [router.isReady, router.asPath, onHome]);
  ...
  return (
    <div className="ai-chatbot">
      {open ? (
        <div className="ai-chatbot__panel" role="dialog" aria-label="University search assistant">
          <div className="ai-chatbot__header">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <h2 className="text-base font-bold">Ask University</h2>
              </div>
            </div>
            <button
              type="button"
      ...
      <button
        ...
        aria-label={open ? 'Close search assistant' : 'Open search assistant'}
        title="Ask University"
      >
