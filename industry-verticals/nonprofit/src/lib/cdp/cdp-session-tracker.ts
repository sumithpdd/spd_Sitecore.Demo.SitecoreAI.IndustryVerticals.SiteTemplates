...
const SESSION_ID_KEY = 'nonprofit-cdp-session-id';
const SESSION_EVENTS_KEY = 'nonprofit-cdp-session-events';
const VISIT_COUNT_KEY = 'nonprofit-cdp-visit-count';
const VISIT_FLAG_KEY = 'nonprofit-cdp-visit-recorded';
...
function derivePageContext(path: string): Record<string, string> {
  ...
  if (normalized.includes('/get-help')) context.category = 'Advice';
  if (normalized.includes('/people')) context.category = 'People';
  if (normalized.includes('/partners')) context.category = 'Partners';
  if (normalized.includes('/appeals')) context.category = 'Appeals';
  if (normalized.includes('/stories')) context.category = 'Stories';
  if (normalized.includes('/donate') || normalized.includes('/fundraise')) context.intent = 'Give';
  if (normalized.includes('jordan-hale')) context.collection = 'Jordan Hale';
...
export function deriveAffinityFromEvents(
  ...
  for (const [key, val] of Object.entries(scores)) {
    ...
    if (['Advice', 'People', 'Partners', 'Appeals', 'Stories'].includes(key)) {
      categories[key] = normalized;
    } else if (key === 'Jordan Hale') {
