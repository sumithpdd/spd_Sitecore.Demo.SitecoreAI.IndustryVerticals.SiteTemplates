<#
.SYNOPSIS
  Upload Capco images to Sitecore Content Hub and emit Image field XML.
...
function New-HubPublicLink {
  ...
  $isVideo = $FileName -match '\.(mp4|webm|mov)$'
  $imageXml = if ($isPdf) {
    "<link text=`"$alt`" linktype=`"external`" url=`"$publicUrl`" anchor=`"`" target=`"_blank`" />"
  } elseif ($isVideo) {
    "<Image src=`"$publicUrl`" dam-id=`"$damId`" alt=`"$alt`" dam-content-type=`"Video`" />"
  } else {
...
$xmlPath = Join-Path $repoMaps 'capco-image-xml.json'
$existingXml = @{}
if (Test-Path $xmlPath) {
  try {
    $parsed = Get-Content $xmlPath -Raw | ConvertFrom-Json
    $parsed.PSObject.Properties | ForEach-Object { $existingXml[$_.Name] = $_.Value }
  } catch { $existingXml = @{} }
}
foreach ($key in $map.Keys) { $existingXml[$key] = $map[$key] }
($existingXml | ConvertTo-Json) | Set-Content $xmlPath -Encoding UTF8
Write-Host "Wrote $csvPath (does not overwrite capco-sitecore-image-field-map.csv)"
