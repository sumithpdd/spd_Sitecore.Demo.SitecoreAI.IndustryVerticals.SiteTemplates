<#
  Stamp Content Hub Brand id 108095 (Capco) on uploaded assets.
...
if (-not (Test-Path $csv)) { throw "Missing $csv - run Upload-CapcoContentHub.ps1 first" }
$rows = Import-Csv $csv
foreach ($row in $rows) {
  $assetId = $row.AssetId
...
Write-Host "Capco brand metadata complete."
